...
class Project extends Framework.Service {
    ...
    async bootstrap() {
        ...
        const setLocale = key => this.app.store.set(`location.${key}`, this.app.options.config.cli.flags[`location.${key}`] ??
    ...
    async searchConfigs() {
        ...
        const findConfig = async function ({ key, searchStrings }) {
            const search = await (0, bud_support_1.globby)(searchStrings, {
