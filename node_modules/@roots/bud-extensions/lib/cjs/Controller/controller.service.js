...
class Controller {
    ...
    get moduleLogger() {
        let logger = new controller_dependencies_1.Signale();
    ...
    setOptions(value) {
        ...
        throw new Error(`[${this.name}] options must be a container or an object`);
    }
    /**
     * Set an extension option
     *
    ...
    async register() {
        ...
        if (this.meta.registered)
            return this;
        this.meta.registered = true;
        await Promise.all(this.app.project.peers.modules[this.name].requires.map(async ([name]) => {
